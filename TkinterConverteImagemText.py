...
#Criando a funcionalidade principal - fazer o upload de um arquivo e depois convertê-lo.
    if filename == '': 
        ...
        p_label_var.set('Imagem carregada com sucesso, pronto para conversão.')
        l.config(fg='#0CDD19',font = ('Times bold', 10))
...
def convert():         
    try: 
        c_label_var.set('Imagem convertida em texto...')
        pytesseract.pytesseract.tesseract_cmd = r'C:\Program Files\Tesseract-OCR\tesseract'
...
f1 = Frame() 
f1.config(bg='white') 
Label(f1, text='Selecionar a imagem', width=20,font=('Times bold', 15), bg='white').pack(side='left') 
Label(f1, text='*formato: png/jpeg', bg='white',font = ('Times bold', 10), width=30).pack(side='right', padx=5) 
...
l = Label(textvariable=p_label_var, fg='red', bg='white',font = ('Times bold', 10)) 
l.pack() 
...
c_label_var.set('O texto da imagem convertida será exibido aqui...') 
c_label = Label(textvariable=c_label_var, font=('Times bold', 13),fg='#433E3B') 
c_label.pack(side='bottom')
Button(root, text='Iniciar conversão', bg='#F58D4B', font=('Times bold', 15), 
